...
namespace faspi
{
    public partial class frmStockTransfer : Form
    {
        ...
        private void frmChallanDetail_Load(object sender, EventArgs e)
        {
            SideFill();
        ...
        private bool validation()
        {
            ...
            if (textBox18.Text == "")
            {
                MessageBox.Show("Enter Unloading Point");
                textBox18.Focus();
                //button1.Focus();
                return false;
            }
        ...
        private void weightCalc()
        {
            ...
            textBox1.Text = ansGridView5.Rows.Count.ToString();
        ...
        private void calcbal()
        {
            ...
            double Lorryfreight = 0;
            double adfreight = 0;
            double balamt = 0;

            Lorryfreight = double.Parse(textBox5.Text);
            adfreight = double.Parse(textBox13.Text);
            balamt = Lorryfreight - adfreight;
            textBox14.Text = funs.DecimalPoint(balamt, 2); 
        }
        private void save()
        {
            if (vid == "0")
            ...
            dtVoucherinfo.Rows[0]["Vt_id"] = 63;
            dtVoucherinfo.Rows[0]["Narr"] = "Stock Transfer";
        ...
        private void SideFill()
        {
            ...
            dtsidefill.Rows[dtsidefill.Rows.Count - 1]["DisplayName"] = "Chng No.";
            dtsidefill.Rows[dtsidefill.Rows.Count - 1]["ShortcutKey"] = "";
            if (Database.utype == "User")
            {
                dtsidefill.Rows[dtsidefill.Rows.Count - 1]["Visible"] = false;
        ...
        public void LoadData(string vi_id, string frmcaption)
        {
            ...
            gStr = vi_id.ToString();
            vid = vi_id;
            vtid = funs.Select_vt_id("Stock Transfer");

            DataTable dtfill = new DataTable();           
           // string str = "SELECT Stocks.GR_id AS Vi_id,dbo.Stocks.Step, CONVERT(nvarchar, VOUCHERINFOs.Vdate, 106) AS Booking_date, VOUCHERINFOs.Vnumber AS GRno, ACCOUNTs_1.name AS Consigner, ACCOUNTs.name AS Consignee,   DeliveryPoints.Name AS Source, DeliveryPoints_1.Name AS Destination, VOUCHERINFOs.DeliveryType, VOUCHERINFOs.PaymentMode AS GR_Type,  VOUCHERINFOs.Transport1 AS Private, VOUCHERINFOs.Transport5 AS Remark, SUM(Voucherdets.Quantity) AS Total_quantity,  SUM(Voucherdets.weight) AS Total_weight, VOUCHERINFOs.Totalamount as total_amount, SUM(Voucherdets.Rate_am) AS Freight, SUM(Voucherdets.exp8amt)   AS door_delivery, CASE WHEN VOUCHERINFOs.PaymentMode = 'FOC' THEN VOUCHERINFOs.Totalamount ELSE 0 END AS total_foc,  CASE WHEN VOUCHERINFOs.PaymentMode = 'Paid' THEN VOUCHERINFOs.Totalamount ELSE 0 END AS total_paid,  CASE WHEN VOUCHERINFOs.PaymentMode = 'To Pay' THEN VOUCHERINFOs.Totalamount ELSE 0 END AS total_pay,   CASE WHEN VOUCHERINFOs.PaymentMode = 'T.B.B.' THEN VOUCHERINFOs.Totalamount ELSE 0 END AS total_Billed FROM DeliveryPoints RIGHT OUTER JOIN VOUCHERINFOs LEFT OUTER JOIN  Voucherdets ON VOUCHERINFOs.Vi_id = Voucherdets.Vi_id ON DeliveryPoints.DPId = VOUCHERINFOs.Consigner_id LEFT OUTER JOIN ACCOUNTs AS ACCOUNTs_1 ON VOUCHERINFOs.Ac_id = ACCOUNTs_1.ac_id LEFT OUTER JOIN ACCOUNTs ON VOUCHERINFOs.Ac_id2 = ACCOUNTs.ac_id LEFT OUTER JOIN  DeliveryPoints AS DeliveryPoints_1 ON VOUCHERINFOs.SId = DeliveryPoints_1.DPId RIGHT OUTER JOIN Stocks ON VOUCHERINFOs.Vi_id = Stocks.GR_id WHERE (Stocks.Godown_id = '" + Database.LocationId + "') GROUP BY Stocks.GR_id, dbo.Stocks.Step,VOUCHERINFOs.Vdate, VOUCHERINFOs.Vnumber, ACCOUNTs_1.name, ACCOUNTs.name, DeliveryPoints_1.Name,   DeliveryPoints.Name, VOUCHERINFOs.DeliveryType, VOUCHERINFOs.PaymentMode, VOUCHERINFOs.Transport1, VOUCHERINFOs.Transport5,  VOUCHERINFOs.Totalamount HAVING (SUM(Stocks.Quantity) > 0) ORDER BY VOUCHERINFOs.Vdate DESC, GRno DESC";
        ...
        private void frmChallanDetail_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
           //string strCombo = "SELECT [name] from DeliveryPoints";
            string strCombo ="SELECT DISTINCT dbo.DeliveryPoints.Name FROM         dbo.Location LEFT OUTER JOIN  dbo.DeliveryPoints ON dbo.Location.Dp_id = dbo.DeliveryPoints.DPId WHERE     (dbo.Location.LocationId <> '"+Database.LocationId+"')";
            string st = e.KeyChar.ToString();
            if (textBox4.Text != "")
            {
                st = textBox4.Text;
        ...
        private void textBox2_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (e.Control && e.KeyCode == Keys.A)
            {
                if (textBox2.Text != "")
                {
                    textBox12.Text = funs.EditAccount(textBox2.Text); ;
                }
            }
        }
        ...
        {
            string strCombo = "select nick_name from Location where nick_name<>'" + Database.LocationNikName + "' order by nick_name";
            string st = e.KeyChar.ToString();
            if (textBox18.Text != "")
            {
                st = textBox18.Text;
