...
namespace faspi
{
    public partial class frm_deliveredby : Form
    {
        DataTable dtItem;
        string dtName;
        ...
        public frm_deliveredby()
        {
            InitializeComponent();
        }

        private void frm_deliveredby_Load(object sender, EventArgs e)
        {
            SideFill();
        }
        public void LoadData(String str, String frmCaption)
        {
            ...
            Database.GetSqlData("select * from " + dtName + " where [D_Id]='" + str + "'", dtItem);
        ...
        private void save()
        {
            ...
            if (gStr == "0")
            {
                ...
                Database.GetSqlData("select count(*) from DeliveredBys where locationid='" + Database.LocationId + "'", dtCount);
                if (int.Parse(dtCount.Rows[0][0].ToString()) == 0)
                {
                    dtItem.Rows[0]["D_Id"] = Database.LocationId + "1";
                    dtItem.Rows[0]["Nid"] = 1;
                ...
                else
                {
                    ...
                    Database.GetSqlData("select max(Nid) as Nid from DeliveredBys where locationid='" + Database.LocationId + "'", dtAcid);
                    int Nid = int.Parse(dtAcid.Rows[0][0].ToString());
                    dtItem.Rows[0]["D_Id"] = Database.LocationId + (Nid + 1);
            ...
            dtItem.Rows[0]["Email"] = textBox4.Text;
        ...
        private bool validate()
        {
            ...
            if (funs.Select_db_id(textBox1.Text) != "" && funs.Select_db_id(textBox1.Text) != gStr)
            {
                MessageBox.Show("DeliveredBy Already Exists");
                return false;
            }
            return true;
        }
        ...
        private void frm_deliveredby_KeyDown(object sender, KeyEventArgs e)
        {
