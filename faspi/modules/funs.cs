...
namespace faspi
{
    class  funs
    {
        ...
        public static DateTime GetIndianTime(DateTime dt)
        {
            return dt;
            //return dt.AddHours(13.5);

            //TimeZoneInfo INDIAN_ZONE = TimeZoneInfo.FindSystemTimeZoneById("India Standard Time");
            //DateTime indianTime = TimeZoneInfo.ConvertTimeFromUtc(dt, INDIAN_ZONE);//(DateTime.UtcNow, INDIAN_ZONE);
        ...
        {
            return Database.GetScalarText("select Path from AccounTypes where name='" + name + "'");
            
        }

        public static string Select_act_under(String name)
        {
            return Database.GetScalarText("select under from AccounTypes where name='" + name + "'");
        }

        public static int Select_act_level(String name)
        {
            return Database.GetScalarInt("select level from AccounTypes where name='" + name + "'");
        }
        ...
        {
            return Database.GetScalarBool("select fixed from AccounTypes where name='" + name + "'");
        }

        public static string Select_act_nature(String name)
        {
            return Database.GetScalarText("select Nature from AccounTypes where name='" + name + "'");
        }
        ...
        {
            DataRow[] dtRows = clsCashing.GetVoucherType().Select("[name]='" + vt_name + "'");
            if (dtRows.Length > 0) { return int.Parse(dtRows[0]["Vt_id"].ToString()); }
            return 0;
        ...
        {
            DataRow[] dtRows = clsCashing.GetVoucherType().Select("Vt_id=" + vt_id);
            if (dtRows.Length > 0) { return int.Parse(dtRows[0]["Numtype"].ToString()); }
            return 0;
        ...
        public static string IndianCurr(double o)
        {
            ...
            //try
            //{
            //    double dval = Convert.ToDouble(o);
            //    System.Globalization.CultureInfo cuInfo = new System.Globalization.CultureInfo("hi-IN");
            //    string strRes = (dval.ToString("C", cuInfo)).Trim();
            //    return strRes.Remove(0, 1).Trim();
            //}
            //catch (Exception ex)
            //{
            //    return "";
        ...
        public static int GenerateVno(int vtid, String dt, string vid)
        {
            ...
            string strSql = "select (Select starting_no from VOUCHERTYPEs where vt_id=" + vtid + ") as startno,";            
            strSql += "(SELECT isnull(Max(Vnumber),0)+1 FROM VOUCHERINFOs, VOUCHERTYPEs WHERE VOUCHERINFOs.Vt_id=VOUCHERTYPEs.Vt_id AND VOUCHERTYPEs.Code= (SELECT VOUCHERTYPEs.Code FROM VOUCHERTYPEs WHERE LocationId='" + Database.LocationId + "' and VOUCHERTYPEs.Vt_id=" + vtid + wherstr + ")) as vno";

            Database.GetSqlData(strSql, dtInfo);
            ...
            prospective = int.Parse(dtInfo.Rows[0]["vno"].ToString());

            if (prospective == 1)
            {
                prospective = int.Parse(dtInfo.Rows[0]["startno"].ToString());// Database.GetScalarInt("Select starting_no from VOUCHERTYPEs where vt_id=" + vtid); 
            ...
            if (numtype == 1)  //yearly
            {
                ...
                //strSql = "select (SELECT Max(VOUCHERINFOs.Vdate) As Vdate FROM VOUCHERINFOs WHERE LocationId='" + Database.LocationId + "' and (((VOUCHERINFOs.Vnumber)<" + prospective + ") AND ((VOUCHERINFOs.Vt_id)=" + vtid + ") and vi_id <>'" + vid + "')) as pre,";
                //strSql += "(SELECT Min(VOUCHERINFOs.Vdate) AS Vdate FROM VOUCHERINFOs WHERE LocationId='" + Database.LocationId + "' and (((VOUCHERINFOs.Vnumber)>" + prospective + ") AND ((VOUCHERINFOs.Vt_id)=" + vtid + ") and vi_id <>'" + vid + "')) as nex";
            ...
            else if (numtype == 2) //monthly
            {
                ...
                strSql = "select (SELECT Max(VOUCHERINFOs.Vdate) As Vdate FROM VOUCHERINFOs WHERE LocationId='" + Database.LocationId + "' and VOUCHERINFOs.Vnumber<" + prospective + " AND VOUCHERINFOs.Vt_id=" + vtid + " and (month(vdate)=" + DateTime.Parse(dt).Month + ") and vi_id <>'" + vid + "') as pre,";
                strSql += "(SELECT Min(VOUCHERINFOs.Vdate) As Vdate FROM VOUCHERINFOs WHERE LocationId='" + Database.LocationId + "' and VOUCHERINFOs.Vnumber>" + prospective + " AND VOUCHERINFOs.Vt_id=" + vtid + " and (month(vdate)=" + DateTime.Parse(dt).Month + ") and vi_id <>'" + vid + "') as nex";
            ...
            //Pre = dtInfo.Rows[0]["pre"].ToString();
            //nex = dtInfo.Rows[0]["nex"].ToString();
            if (Pre == "" && nex == "")
            {
                return prospective;
            }
